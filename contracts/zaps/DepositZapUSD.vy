...
BASE_LP_TOKEN: constant(address) = 0xb9d696ECba4A545a887e3e5FA01fbf32582B926C
...
def add_liquidity(
    ...
    if _deposit_amounts[0] != 0:
        coin: address = KaglaMeta(_pool).coins(0)
    ...
    if deposit_base:
        ...
        KaglaBase(BASE_POOL).add_liquidity(base_amounts, 0)
    ...
    return KaglaMeta(_pool).add_liquidity(meta_amounts, _min_mint_amount, _receiver)
...
def remove_liquidity(
    ...
    # Withdraw from meta
    meta_received: uint256[N_COINS] = KaglaMeta(_pool).remove_liquidity(
        _burn_amount,
    ...
    KaglaBase(BASE_POOL).remove_liquidity(meta_received[1], min_amounts_base)
...
def remove_liquidity_one_coin(
    ...
    if i == 0:
        coin_amount = KaglaMeta(_pool).remove_liquidity_one_coin(_burn_amount, i, _min_amount, _receiver)
    else:
        base_coins: address[BASE_N_COINS] = BASE_COINS
        coin: address = base_coins[i - MAX_COIN]
        # Withdraw a base pool coin
        coin_amount = KaglaMeta(_pool).remove_liquidity_one_coin(_burn_amount, MAX_COIN, 0, self)
        KaglaBase(BASE_POOL).remove_liquidity_one_coin(coin_amount, i-MAX_COIN, _min_amount)
...
def remove_liquidity_imbalance(
    ...
    if withdraw_base:
        amounts_meta[MAX_COIN] = KaglaBase(BASE_POOL).calc_token_amount(amounts_base, False)
        amounts_meta[MAX_COIN] += amounts_meta[MAX_COIN] * fee / FEE_DENOMINATOR + 1

    # withdraw from metapool and return the remaining LP tokens
    burn_amount: uint256 = KaglaMeta(_pool).remove_liquidity_imbalance(amounts_meta, _max_burn_amount)
    ...
    if withdraw_base:
        ...
        if leftover > 0:
            ...
            if not self.is_approved[coin][_pool]:
                ERC20(coin).approve(_pool, MAX_UINT256)
                self.is_approved[coin][_pool] = True
            burn_amount -= KaglaMeta(_pool).add_liquidity([convert(0, uint256), leftover], 0, msg.sender)
...
def calc_withdraw_one_coin(_pool: address, _token_amount: uint256, i: int128) -> uint256:
    ...
    """
    if i < MAX_COIN:
        return KaglaMeta(_pool).calc_withdraw_one_coin(_token_amount, i)
    else:
        _base_tokens: uint256 = KaglaMeta(_pool).calc_withdraw_one_coin(_token_amount, MAX_COIN)
        return KaglaBase(BASE_POOL).calc_withdraw_one_coin(_base_tokens, i-MAX_COIN)
...
def calc_token_amount(_pool: address, _amounts: uint256[N_ALL_COINS], _is_deposit: bool) -> uint256:
    ...
    base_tokens: uint256 = KaglaBase(BASE_POOL).calc_token_amount(base_amounts, _is_deposit)
    meta_amounts[MAX_COIN] = base_tokens

    return KaglaMeta(_pool).calc_token_amount(meta_amounts, _is_deposit)
