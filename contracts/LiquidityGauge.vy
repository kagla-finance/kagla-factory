...
@notice Implementation contract for use with Kagla Factory
"""
...
MINTER: constant(address) = 0x210c5BE93182d02A666392996f62244001e6E04d
...
def initialize(_lp_token: address):
    ...
    self.inflation_rate = KGL20(KGL).rate()
    self.future_epoch_time = KGL20(KGL).future_epoch_time_write()
...
def _update_liquidity_limit(addr: address, l: uint256, L: uint256):
    """
    @notice Calculate limits which depend on the amount of KGL token per-user.
            Effectively it calculates working balances to apply amplification
            of KGL production by KGL
...
def set_killed(_is_killed: bool):
    ...
    @dev When killed, the gauge always yields a rate of 0 and so cannot mint KGL
