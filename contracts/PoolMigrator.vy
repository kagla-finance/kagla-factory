...
@notice Zap for moving liquidity between Kagla Factory pools in a single transaction
...
def migrate_to_new_pool(_old_pool: address, _new_pool: address, _amount: uint256) -> uint256:
    ...
    @dev Each pool must be deployed by the Kagla Factory and contain identical
