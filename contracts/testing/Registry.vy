...
def _get_rates(_pool: address) -> uint256[MAX_COINS]:
    ...
    if base_pool == ZERO_ADDRESS:
        ...
        rates[base_coin_idx] = KaglaPool(base_pool).get_virtual_price()
        for i in range(MAX_COINS):
            if i == base_coin_idx:
                break
            rates[i] = 10 ** 18
...
def _get_balances(_pool: address) -> uint256[MAX_COINS]:
    ...
    for i in range(MAX_COINS):
        ...
        if is_v1:
            balances[i] = KaglaPoolV1(_pool).balances(i)
        else:
            balances[i] = KaglaPool(_pool).balances(convert(i, uint256))
...
def _get_meta_underlying_balances(_pool: address, _base_pool: address) -> uint256[MAX_COINS]:
    ...
    if base_total_supply > 0:
        underlying_pct = KaglaPool(_pool).balances(base_coin_idx) * 10**36 / base_total_supply

    for i in range(MAX_COINS):
        if self.pool_data[_pool].ul_coins[i] == ZERO_ADDRESS:
            break
        if i < base_coin_idx:
            ul_balance = KaglaPool(_pool).balances(i)
        else:
            if is_v1:
                ul_balance = KaglaPoolV1(_base_pool).balances(convert(i - base_coin_idx, int128))
            else:
                ul_balance = KaglaPool(_base_pool).balances(i-base_coin_idx)
...
def get_virtual_price_from_lp_token(_token: address) -> uint256:
    ...
    """
    return KaglaPool(self.get_pool_from_lp_token[_token]).get_virtual_price()


@view
@external
def get_A(_pool: address) -> uint256:
    return KaglaPool(_pool).A()
...
def get_parameters(_pool: address) -> PoolParams:
    ...
    pool_params.A = KaglaPool(_pool).A()
    pool_params.future_A = KaglaPool(_pool).future_A()
    pool_params.fee = KaglaPool(_pool).fee()
    pool_params.future_fee = KaglaPool(_pool).future_fee()
    pool_params.admin_fee = KaglaPool(_pool).admin_fee()
    pool_params.future_admin_fee = KaglaPool(_pool).future_admin_fee()
    pool_params.future_owner = KaglaPool(_pool).future_owner()

    if self.pool_data[_pool].has_initial_A:
        pool_params.initial_A = KaglaPool(_pool).initial_A()
        pool_params.initial_A_time = KaglaPool(_pool).initial_A_time()
        pool_params.future_A_time = KaglaPool(_pool).future_A_time()
...
def get_fees(_pool: address) -> uint256[2]:
    ...
    """
    return [KaglaPool(_pool).fee(), KaglaPool(_pool).admin_fee()]
...
def _get_new_pool_coins(
    ...
    for i in range(MAX_COINS):
        ...
        if _is_underlying:
            if _is_v1:
                coin = KaglaPoolV1(_pool).underlying_coins(convert(i, int128))
            else:
                coin = KaglaPool(_pool).underlying_coins(i)
            self.pool_data[_pool].ul_coins[i] = coin
        else:
            if _is_v1:
                coin = KaglaPoolV1(_pool).coins(convert(i, int128))
            else:
                coin = KaglaPool(_pool).coins(i)
...
def add_metapool(
    ...
    if base_pool == ZERO_ADDRESS:
        base_pool = KaglaMetapool(_pool).base_pool()
...
def set_pool_asset_type(_pool: address, _asset_type: uint256):
    """
    @notice Set the asset type name for a kagla pool
