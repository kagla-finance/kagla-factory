...
interface KaglaFactoryMetapool:
...
def get_metapool_rates(_pool: address) -> uint256[2]:
    ...
    rates[1] = KaglaPool(self.pool_data[_pool].base_pool).get_virtual_price()
    return rates
...
def get_balances(_pool: address) -> uint256[MAX_PLAIN_COINS]:
    ...
    """
    if self.pool_data[_pool].base_pool != ZERO_ADDRESS:
        return [KaglaPool(_pool).balances(0), KaglaPool(_pool).balances(1), 0, 0]
    ...
    for i in range(MAX_PLAIN_COINS):
        if i < n_coins:
            balances[i] = KaglaPool(_pool).balances(i)
        else:
            balances[i] = 0
    return balances
...
def get_underlying_balances(_pool: address) -> uint256[MAX_COINS]:
    ...
    underlying_balances[0] = KaglaPool(_pool).balances(0)

    base_total_supply: uint256 = ERC20(self.pool_data[_pool].coins[1]).totalSupply()
    if base_total_supply > 0:
        underlying_pct: uint256 = KaglaPool(_pool).balances(1) * 10**36 / base_total_supply
        base_pool: address = self.pool_data[_pool].base_pool
        ...
        for i in range(MAX_COINS):
            if i == n_coins:
                break
            underlying_balances[i + 1] = KaglaPool(base_pool).balances(i) * underlying_pct / 10**36
...
def get_admin_balances(_pool: address) -> uint256[MAX_PLAIN_COINS]:
    ...
    for i in range(MAX_PLAIN_COINS):
        if i == n_coins:
            break
        admin_balances[i] = KaglaPool(_pool).admin_balances(i)
...
def deploy_plain_pool(
    ...
    @param _fee Trade fee, given as an integer with 1e10 precision. The
                ...
                50% of the fee is distributed to veKGL holders.
    ...
    KaglaPlainPool(pool).initialize(_name, _symbol, _coins, rate_multipliers, _A, _fee)
...
def deploy_metapool(
    ...
    pool: address = create_forwarder_to(implementation)
    KaglaPool(pool).initialize(_name, _symbol, _coin, 10 ** (36 - decimals), _A, _fee)
...
def add_existing_metapools(_pools: address[10]) -> bool:
    ...
    for pool in _pools:
        ...
        meta_coin: address = KaglaFactoryMetapool(pool).coins(0)
